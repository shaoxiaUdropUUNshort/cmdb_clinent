...
class Logger(object):
    ...
    def __init__(self):
        ...
        self.run_logger = None  # 经过 self.initialize_run_log() 变成了一个logger 对象
        self.error_logger = None  # self.initialize_error_log()  变成一个logger对象

        self.initialize_run_log()
        self.initialize_error_log()

    def __new__(cls, *args, **kwargs):
        # python 单例模式的应用。 不管在哪里调用，永远都使用一个对象进行操作。这样就不会发生多处程序，同时调用时
        # 对象冲突的问题。
